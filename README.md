...
```json
{
  ...
  "type": "recording_stopped | screen_share_stopped | invalid_share_surface | recording_error | ip_changed | tab_hidden | tab_away | disconnected | peer_copy_cluster | recurring_pair | web_paste | first_attempt_solve | tough_first_attempt",
...
|---|---|---|---|
| `ALERTS_INGEST_API_KEY` | Yes | none | Shared secret for `POST /api/alerts` (`x-api-key`). Unset = reject all ingest. the operator sets the real value at deploy time; the test suite uses a throwaway placeholder. Generate with `openssl rand -base64 32`. |
...
> `.env.deploy.example` carry placeholders only; the operator supplies the real key
