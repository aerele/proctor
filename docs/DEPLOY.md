...
> table. The live dev stack (`${PROJECT_ID}`, api rev `proctor-api-00006-pjr`
...
```bash
gcloud builds submit backend  --tag asia-south1-docker.pkg.dev/${PROJECT_ID}/proctor/api:latest --async
gcloud builds submit frontend --tag asia-south1-docker.pkg.dev/${PROJECT_ID}/proctor/web:latest --async
