...
- **Alert → recording deep-links resolve to "No recording attached".** The sure-shot alert deep-link target is `sureShotVideoKey` (`backend/src/proctorAlerts.mjs`), which returns `session.merged_video_key || null`. Since no producer ever writes `merged_video_key`, that field is **always null**, so in the **Live alerts** console alerts whose session has no resolvable merged video show **"No recording attached"** rather than linking into this surface. This is the standing behavior, not a pending item. (Recordings themselves are reviewed via the chunk player here; the alert deep-link does not jump into it.)
