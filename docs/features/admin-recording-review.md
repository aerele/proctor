...
> **Product context.** Aerele Proctor is now a standalone own-editor exam platform: candidates code in our React + Monaco workspace with Judge0-backed Run/Submit. The recording-review surface plays back the screen/camera chunks those candidate sessions uploaded. The **submission-time markers** described below come from legacy `proctor_submission_events` docs if present, otherwise from the proctor's own in-app submissions (`proctor_submissions`); the HackerRank poller that used to POST them was removed when proctor moved to its own in-app contest platform.
...
- **Playback is chunk-based by design.** Recordings are stored as a sequence of 30-second `.webm` chunks and the player plays them **in order, auto-advancing** between chunks; there is **no merged-video producer** that stitches them into one continuous file. Playback is seamless-ish via preloading, but it is chunk playback, not a single merged file. (Verified: no merged-video producer exists in `backend/src/`; a `merged.webm` path appears only in an old planning doc.)
...
- **Submission markers come from legacy poller-mirror docs or the native fallback.** Markers appear when legacy `proctor_submission_events` docs exist for that user/contest (mirrored from the old HackerRank poller, which has been removed), or from the native `proctor_submissions` fallback for own-editor exams. The poller ingest itself was removed; the own-editor path shows the markers from native submissions — hence "no submission events" in the older screenshots above.
