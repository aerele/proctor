...
Run: `cd <repo>/backend && node --test test/roster.test.mjs`
...
Run the full backend suite (no regressions): `cd <repo>/backend && node --test test/*.test.mjs`
Expected: all tests pass (the pre-existing suites — phase2, alerts, exec, editorEvents, judge0Adapter, sanitize — stay green).

- [ ] **Step 1.5: Commit**

```bash
cd <repo> && git add backend/src/handler.mjs backend/test/roster.test.mjs && git commit -m "S2: backend roster store (upload/get/clear) + rooms in settings
...
Run: `cd <repo>/backend && node --test test/*.test.mjs` — all green.

- [ ] **Step 2.5: Commit**

```bash
cd <repo> && git add backend/src/handler.mjs backend/test/roster.test.mjs && git commit -m "S2: public exam-config + roster unique-ID lookup (masked, minimal fields)
...
Run: `cd <repo>/backend && node --test test/*.test.mjs`
...
```bash
cd <repo> && git add backend/src/handler.mjs backend/test/roster.test.mjs && git commit -m "S2: enforce roster match at /api/session/start (server-side identity override)
...
Run: `cd <repo>/frontend && npx vitest run src/roster/parseRoster.test.ts`
...
Also run the whole frontend unit suite: `cd <repo>/frontend && npm test` — all green (includes coding/editorEvents tests).

- [ ] **Step 4.5: Commit**

```bash
cd <repo> && git add frontend/src/roster/parseRoster.ts frontend/src/roster/parseRoster.test.ts && git commit -m "S2: pure CSV/TSV roster parser + column-mapping heuristics (vitest)
...
Run: `cd <repo>/frontend && npm run lint`
Expected: `tsc -b` exits 0 (no output). Then `npm test` — all vitest suites still green.

- [ ] **Step 5.5: Commit**

```bash
cd <repo> && git add frontend/src/types.ts frontend/src/api.ts frontend/src/App.tsx && git commit -m "S2: roster api client + types + demo-mode roster gate parity
...
```bash
cd <repo> && git add frontend/src/App.tsx && git commit -m "S2: admin settings — rooms list field + candidate roster upload section
...
Run: `cd <repo>/frontend && npm run lint` — exits 0.
Run: `cd <repo>/frontend && npm test` — all green.
Run: `cd <repo>/frontend && npm run build` — builds cleanly.

- [ ] **Step 7.9: Commit**

```bash
cd <repo> && git add frontend/src/App.tsx && git commit -m "S2: student login — unique-ID-confirm roster gate + room dropdown (+Other)
...
```bash
cd <repo>/backend && node --test test/*.test.mjs
cd <repo>/frontend && npm test && npm run lint && npm run build
```
...
```bash
cd <repo>/frontend && VITE_DEMO_MODE=true VITE_ADMIN_PASSWORD=admin npm run dev
```
...
```bash
cd <repo> && git add night-run/MORNING-NOTES.md && git commit -m "S2: record roster-login build + integration-test results in morning notes
