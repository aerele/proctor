...
Companion to `2026-06-11-architecture-decomposition.md` (the original plan). This reconciles that plan against the current code after the exam-eve work, per the operator's instruction (re-read → reconcile → update → only then execute, behavior-preserving + product-usable at each step). Branch: `refactor/decomp-resume` (cut from deployed master `f7a59ff`).
...
| B14 | adminSessions 3023-3359 + 3 raw-where + final scopingLint re-pin to `{}` | **Highest** | **MORNING — the operator's call on end-state** |
| B15 | mop-up (handler → dispatcher + wiring + conventions doc) | Low | after B14 |
| Frontend F0-F6 + api A0-A9 | separate session; F6/F6a lazy-load = perf win but **product-risk near live exam** | — | **MORNING (F6 needs the operator: no live exam window)** |

## Tonight's autonomous scope
Execute **B2, B3, B5, B6, B7** on this branch (behavior-preserving, leaf, own test files, no raw-where, no exam-critical helpers; "obvious fixes: just do"). One at a time, full suite + 4 guards green per phase, commit each. Everything else → morning ratification + merge (the operator's calls flagged above). The branch stays UNMERGED (does not touch the deployed/exam code) until the operator ratifies.
...
Branch `refactor/decomp-resume` pushed to origin, UNMERGED — for the operator's morning ratification + merge.

REMAINING (need the operator's input — see Open questions): **B4**, **B8–B15** (esp. B9/B10 enforcement+alerts shared-helper threading; B13/B14 raw-where migration + scopingLint re-pin end-state), frontend **F0–F6**, api **A0–A9**.

## 🔁 ON RESUME (the operator instruction, 2026-06-18 — refactor SLATED, likely tomorrow after the test, before release)
...
## Open questions for the operator (morning)
