...
- Old playbook: `<your-contest-eval-data-dir>/{METHODOLOGY.md,ANGLES.md,ADDITIONAL-ANGLES.md,CONVENTIONS.md}`, scripts (`analyze_meta.py`, `clone_detect.py`, `build_profiles.py`, `build_report.py`, `reconcile.py`), and run outputs from an earlier live contest-eval engagement.
...
| B4 | Honest failed reach ("hidden gems") | DET+ triage + LLM verify | A lesson from earlier real runs (most apparent "gems" turned out to be partial-score gamers) is now largely *deterministic*: stub-delta + active-effort + typed-ratio expose hardcode-stub gaming (D12/D13). LLM (J6) verifies reasoning quality on survivors. |
...
| D12 | Stub-delta | Final code vs stub: line-level edit distance. `score > 0 AND delta < 10 lines` ⇒ `partial_gamer` flag (formalizes the gem-gamer lesson from earlier real runs). |
...
| `gem-verify` (J6) | Honest reach genuine? (real-run lesson) | D13 gem candidates surviving D12 stub-delta filter | unsolved-problem code + replay digest + partial-score breakdown | `real_reasoning \| partial_gamer \| inconclusive` |
...
Pure function `computeRecommendedAction(scorecard)` in `evaluationMetrics.mjs`, recommendation vocabulary and precedence (copy > AI > talent):
...
1. **Action vocabulary.** Keep the existing 7-action set (`REJECT-CHEAT / PEN-PAPER / AI-SUSPECT / ADVANCE / VERIFY-DESK / REJECT / NOT-ADVANCED`)? Is there still a pen-and-paper / desk-verify round in the new pipeline, or should those collapse into a single `INVESTIGATE`?
