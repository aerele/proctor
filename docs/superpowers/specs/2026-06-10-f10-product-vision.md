...
candidate types unique_id (label-driven prompt, e.g. "Roll Number")
  ...
  → (username_norm, contest_slug) = (person × contest)     // the operator's locked composite key,
...
| F5.2 integrity-checkpoint investigation | C6/C10 | KEPT-OPEN: remove if click-only/no-signal — discuss with the operator (in-flight task) |
...
| ROADMAP 6.1 WebSockets vs 5s polling | S2/A7 | surfaced to the operator (Q5) — keep-dead or backlog |
...
## 10. ANSWERS — the operator, 2026-06-10 ~20:00 IST (voice). §9 is CLOSED.
