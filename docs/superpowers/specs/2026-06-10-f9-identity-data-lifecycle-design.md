...
node:test, in-memory Firestore/GCS fakes per existing conventions (155 passing today). Frontend: tsc + build + label-rendering tests; validate-before-send on anything shown to the operator.
...
Infra smoke per stage on the dev project (${PROJECT_ID}): catch Firestore composite-index `FAILED_PRECONDITION` early (new `contest_slug ==` combos).
