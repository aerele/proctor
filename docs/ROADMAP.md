...
Status: **requirements capture only.** This is the organized version of the operator's feature dump.
...
> the operator: *"I want to go for a test. Just don't go and build it. Maybe you can build a test page for me separately just to see the feasibility."*
...
**Open decision (2.x):** how much *test state* to preserve vs. just the session/auth. the operator: session preservation is the floor; actual test-state restoration is "discuss later."

---

## Epic 3 — Student UX / guided flow

> the operator (follow-up): the login/onboarding UX must be **really well done** — at every step the student clearly knows *what's happening, what to do now, and what's next*, so they can self-serve without asking staff.
...
## Epic 4 — Admin live monitoring & alerts  *(the operator's "heading 1", more depth later)*
...
## Epic 5 — Live submission evaluation & anti-cheat during the test  *(the operator's "heading 2", more depth later)*

| # | Want |
|---|------|
| 5.1 | **Evaluate submissions as they arrive**, not post-contest. An external poller (the operator's laptop) polls HackerRank for new submissions, downloads them, and evaluates immediately against the existing contest-eval criteria. *(the operator will point to the files that show how to poll + evaluate — see `contest-eval/` methodology.)* |
...
| 6.2 | **Inbound alerts API + API keys:** the admin panel exposes an authenticated API and can **generate an API key** that the operator stores in his laptop's Claude Code. The external evaluator **pushes alert payloads** (a user's evaluation-so-far, who-copied-whom, full details) into the panel for action. |
...
```
the operator's laptop                          Proctor backend (this app)          Admin (phone/laptop)
┌─────────────────────────┐              ┌──────────────────────────┐        ┌───────────────────┐
...
> the operator (voice, 2026-06-18): an idea to capture, NOT to build yet — decide if it's worth it first.
...
**Open decision — is it worth it?** the operator is unsure: every code change would require rewriting/maintaining the suite, and if unit coverage is already solid the marginal confidence may not justify the maintenance. DISCUSS FIRST: (a) marginal value over the existing unit tests; (b) realistic per-change maintenance burden; (c) scope — which flows genuinely need cross-browser/concurrent E2E vs. plain unit coverage. **Build only if we conclude it's worth the effort; do not build without that discussion.**
