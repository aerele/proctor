...
Run: `cd <repo>/backend && node --test test/judge0Adapter.test.mjs`
...
Run: `cd <repo>/backend && node --test test/exec.test.mjs`
...
Run: `cd <repo>/backend && node --test test/editorEvents.test.mjs`
Expected: FAIL — 404.
...
```bash
cd <repo>/backend && npm test   # all pass
cd <repo>
...
Run: `cd <repo>/frontend && npm i -D vitest`
...
Run: `cd <repo>/frontend && npx vitest run src/coding/editorEvents.test.ts`
...
```bash
cd <repo>/frontend && npx tsc --noEmit   # exit 0
...
Run: `cd <repo>/frontend && npm i @monaco-editor/react monaco-editor`
...
Run: `cd <repo>/frontend && npx tsc --noEmit && npm run build`
...
Run: `cd <repo>/frontend && npx vitest run && npm run build`
Expected: vitest passes, build clean.

- [ ] **Step 3: Demo-mode visual run-through** (no Judge0 key needed — `VITE_DEMO_MODE=true`)

Run: `cd <repo>/frontend && VITE_DEMO_MODE=true VITE_ADMIN_PASSWORD=dev npm run dev`
...
- **Open items deferred to the operator/plan-time (not blockers):** exact Judge0 `language_id`s (verify via `GET /languages`); whether to show candidates which hidden test failed (currently pass/fail counts only — matches "don't leak inputs").

---

## Execution options (after the operator approves)
1. **Subagent-driven (recommended)** — fresh subagent per task, review between tasks.
2. **Inline execution** — batch with checkpoints.
Do NOT start execution until the operator reviews this plan + the design doc.
