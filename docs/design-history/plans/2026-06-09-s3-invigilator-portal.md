...
Run: `cd <repo>/backend && node --test test/invigilator.test.mjs`
...
Run: `cd <repo>/backend && node --test test/invigilator.test.mjs` → all Task-1 tests pass.
Run: `cd <repo>/backend && npm test` → ALL backend tests pass (existing suites unaffected).
...
Run: `cd <repo>/backend && node --test test/invigilator.test.mjs`
Run: `cd <repo>/backend && npm test` (especially `exec.test.mjs` must still pass)
...
Run: `cd <repo>/frontend && npx vitest run src/invigilator/gateLogic.test.ts`
...
Run: `cd <repo>/frontend && npx vitest run src/invigilator/gateLogic.test.ts` → 4 test groups pass.
Run: `cd <repo>/frontend && npm run lint` → no TS errors. (Note: `npm run test` runs ALL vitest files — also fine to use.)
...
Run: `cd <repo>/frontend && npm run lint` → no errors.
Run: `cd <repo>/frontend && npm run test` → all vitest suites pass.
...
Run: `cd <repo>/frontend && npm run lint` → no errors.
Run: `cd <repo>/frontend && npm run test` → all pass.
...
cd <repo>/frontend && npm run test && npm run lint
```
...
```bash
cd <repo>/frontend && VITE_DEMO_MODE=true VITE_ADMIN_PASSWORD=dev VITE_INVIGILATOR_PASSWORD=invig npm run dev
```
