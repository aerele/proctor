...
- [ ] **Step 0.1:** Run:
```bash
cd <repo> && grep -c "getRosterMeta\|normalizeUniqueId\|ROSTER_COLLECTION\|ROSTER_LIMIT" backend/src/handler.mjs
```
...
- [ ] **Step 0.2:** Run:
```bash
cd <repo>/backend && node --test test/roster.test.mjs
```
...
Run: `cd <repo>/backend && node --test test/attendance.test.mjs`
...
```bash
cd <repo> && git add backend/src/handler.mjs backend/test/attendance.test.mjs && git commit -m "S6: GET /api/admin/attendance — roster taken/not-taken + absentee list
...
Run: `cd <repo>/frontend && npx vitest run src/attendance/computeAttendance.test.ts`
...
Run: `cd <repo>/frontend && npm test`
Expected: ALL frontend tests pass.

- [ ] **Step 2.5: Commit**

```bash
cd <repo> && git add frontend/src/attendance && git commit -m "S6: pure attendance math + absentees CSV builder (vitest)
...
Run: `cd <repo>/frontend && npx tsc -b --pretty false`
...
```bash
cd <repo> && git add frontend/src/api.ts && git commit -m "S6: fetchAttendance API client + roster_unique_id on demo sessions
...
```bash
cd <repo> && git add frontend/src/App.tsx && git commit -m "S6: admin Attendance tab — counts, unmatched note, absentee table + CSV
...
- [ ] **Step 5.6: Evidence.** Take a screenshot of the full Attendance tab (the Step-5.4 or 5.5 state) and save it as `<repo>/night-run/evidence/s6-attendance-demo.png`. Visually inspect it yourself before counting this step done (counts readable, table populated).
...
```bash
cd <repo> && git add night-run/evidence/s6-attendance-demo.png && git commit -m "S6: demo-mode browser integration evidence for the Attendance tab
