...
operator-approved (no approval gate; behavior-preserving; suite green at EVERY step). Execute AFTER the E2E green baseline. Goals (priority): (1) easy for AI to code, (2) maintainability, (3) performance, (4) security. Scope = the 3 god-files only; leave the already-organized domain modules/folders alone.
