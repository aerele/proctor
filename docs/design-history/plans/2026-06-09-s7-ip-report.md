...
**Spec:** `docs/design-history/specs/2026-06-09-s7-ip-report-design.md` — read it first.
...
```bash
cd <repo>/backend && node --test test/ipReport.test.mjs
```
...
```bash
cd <repo> && git add backend/src/ipReport.mjs backend/test/ipReport.test.mjs && git commit -m "feat(s7): pure IP-wise session aggregation module (buildIpReport)"
```
...
```bash
cd <repo> && git add backend/src/handler.mjs backend/test/ipReport.test.mjs && git commit -m "feat(s7): GET /api/admin/ip-report + trust the last x-forwarded-for hop"
```
...
```bash
cd <repo>/frontend && npx vitest run src/ipReport.test.ts
```
...
```bash
cd <repo> && git add frontend/src/ipReport.ts frontend/src/ipReport.test.ts frontend/src/types.ts && git commit -m "feat(s7): frontend IP-report types + pure grouping module (vitest)"
```
...
```bash
cd <repo>/frontend && npm run lint && npm test
```
...
```bash
cd <repo> && git add frontend/src/api.ts && git commit -m "feat(s7): fetchIpReport with demo-population IP assignment"
```
...
```bash
cd <repo>/frontend && npm run lint && npm test && npm run build
```
...
```bash
cd <repo> && git add frontend/src/App.tsx && git commit -m "feat(s7): admin IP-report tab (clusters-first table, scope toggle)"
```
...
```bash
cd <repo>/frontend && VITE_DEMO_MODE=true VITE_ADMIN_PASSWORD=demo-admin npm run dev
```
...
```bash
cd <repo> && git add night-run/evidence/s7-ip-report.png night-run/evidence/s7-ip-report-all.png night-run/MORNING-NOTES.md && git commit -m "test(s7): browser-verified IP report in demo mode + morning notes"
```
