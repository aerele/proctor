...
- HR dependency classes: UI labels (~50 strings, RENAME-ONLY), `hackerrank_username` field in form/session/DTOs (rename w/ dual-read adapters), `contest_url` + `contestSlugFromUrl` (DEAD — the operator: name → slug, URL obsolete), `username_norm` (KEEP — it's an internal normalized key, not HR-specific).

## Decisions (defaults chosen for the operator's stated scenario: 2 colleges running in parallel)
