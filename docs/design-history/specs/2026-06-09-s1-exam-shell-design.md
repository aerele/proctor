...
On the first anomaly of an episode: the bar unmounts **completely** — no red bar, no placeholder; the page top is plain background (per the operator's locked intent: bar presence = all good, absence = anomaly; an imitation overlay can fake a red strip more easily than it can fake an absence the invigilator was told to look for). Simultaneously a full-width red **AnomalyPanel** appears at the top of the content flow.
...
- Adding `fullscreen_exit` (or `topbar_hidden`) to backend `SURE_SHOT_EVENT_TYPES` so admins get alerts — a deliberate ~5-line follow-up once the operator confirms desired severity/noise; tonight the events are stored but raise no alerts.
