...
3. **The gate is opt-in per contest** via a new boolean `room_gate_enabled` on the existing settings doc (default **false** → zero behavior change for current flows, Slice-1 tests, and existing live runs until an admin turns it on). It must be ON before invigilators can release codes.
