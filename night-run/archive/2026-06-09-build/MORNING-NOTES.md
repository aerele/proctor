...
- **Demo-mode browser walkthrough PASSED on :9222** (evidence: night-run/evidence/s2-verify-01…10.png), all 6 plan checks:
  ...
  3. Student page: "STEP 1 — CONFIRM YOUR IDENTITY" with label "Roll No"; wrong ID 99XX999 → "could not find that ID" error; lowercase `<test-roll>` → confirm card `<TEST-ROLL>` / Asha Raman / `as**@example.com` / Lab A-1 (lowercase→uppercase normalization proven). **DOM-asserted: raw email + phone appear NOWHERE in the page HTML; only the masked email renders.**
