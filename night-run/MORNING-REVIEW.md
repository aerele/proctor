...
5. **B4 — sure-shot video deep-link cross-bucket signing (GCP).** The video-worker merges clips into `DEST_BUCKET` (review videos) and writes `merged_video_key` back onto the session doc, but the backend signs the alert's `video_key` against `EVIDENCE_BUCKET`. If those two buckets differ, the "Open evidence clip" link on a sure-shot alert will 404. Logic is implemented + unit-tested (no-merge → link hidden; merged → deep-links to the object); flagged with a `NOTE (morning/GCP validation)` in `video-worker/src/server.mjs`. **Decide: merge into `EVIDENCE_BUCKET`, or teach the backend the review-video bucket.** Also confirm the worker's Firestore write-back + `datastore.user` IAM grant works on real GCP.
...
**🟡 C1 — ADDRESSED per your call (plain password no longer in the JS).** Your decision: don't introduce new auth now; just make sure the plain admin password isn't extractable from the JS. **Done** — the deployed frontend now embeds only a **SHA-256 hash** (`VITE_ADMIN_PASSWORD_HASH`); the unlock gate hashes the typed password and compares to the hash; the typed password (held in memory) is what's sent to the backend's existing `x-admin-password` check. `deploy-gcp.sh` computes the hash and no longer passes the plain value. (Local dev keeps the plain `VITE_ADMIN_PASSWORD=dev` fallback.) Residual risk you accepted for today: the backend is still gated by a single shared password (not leaked, but brute-forceable in theory) — fine for a contest that isn't being pen-tested. Full staff-auth (IAP/SSO) remains the long-term upgrade if you ever want it.
