...
---
## S4 problem authoring — BUILT + SUITES VERIFIED (worktree feat/s4-problem-authoring)
- **All 8 build tasks done via TDD** (commits 41e4506 → a043a9b): Firestore-backed problem bank module (validation, per_test / all_or_nothing scoring, async `getProblem` with seed fallback); admin problem CRUD endpoints (list/get/upsert/delete) + bank store wiring; active-problem assignment in settings + public problem (no hiddenTests) in start/resume payload; exec endpoints read the bank + submit-time scoring; frontend types + pure problem-draft form logic; api client + demo-mode store/active problem; admin "Problems" tab (author/publish/delete + set-active); server-driven candidate problem replaces `SLICE1_PROBLEM` (generic starters, samples, score display).
- **Full suites green at branch HEAD (a043a9b + notes commit):** backend 293/293 (node:test), frontend vitest 128/128 (6 files) + `tsc --noEmit` clean + `npm run build` clean.
- **Demo-mode browser walkthrough: DEFERRED TO POST-MERGE** — the :9222 browser belongs to another lane tonight; this worktree lane ran suites only. Run the plan's Task 9 Step 2 checklist (Problems tab authoring → set active → student sees published problem + generic starter → Run/Submit score → unpublish → contest-link fallback) on :9222 after merge.
- **Real-backend smoke (Task 9 Step 3): DEFERRED-TO-MORNING** — no local-backend harness was running at verification time (only the :5173 vite dev server, another lane's); did not stand up GCP for this, per the plan. Morning: author a problem via `POST /api/admin/problems`, set `problem_id` via settings, confirm `/api/session/resume` returns `problem` without `hiddenTests`.
- No screenshots for S4 yet (browser verify deferred), so no `night-run/evidence/s4-*.png` — capture them during the post-merge walkthrough.
