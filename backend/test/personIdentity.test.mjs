...
// case (~700 self-entering students). Karthi's identity model: uniqueness key =
...
test("NO-ROSTER re-login: replay returns the SAME session; concurrent 2nd login WAITS (pending) on the SAME identity; resume re-enters; one username_norm", async () => {
  ...
  const first = await call(startReq({
    ...
    name: "Kishore", email: "kishore@x.com", consent_accepted: true
  }));
  ...
  assert.equal(firstDoc.username_norm, "23cs091");
  assert.equal(secondDoc.username_norm, "23cs091"); // SAME identity key
