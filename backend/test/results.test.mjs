...
test("buildResultsRows: scoreboard identities with no enrollment ride as flagged unmatched rows (never dropped)", () => {
  ...
  fx.submissions = [
    ...fx.submissions,
    { username_norm: "23cs200", person_id: null, candidate_id: "23CS200", problem_id: "p1", score: 100, max_points: 100, created_at: "2026-06-10T04:30:00.000Z" },
    { username_norm: "23cs200", person_id: null, candidate_id: "23CS200", problem_id: "p2", score: 50, max_points: 100, created_at: "2026-06-10T04:40:00.000Z" }
  ];
  const sessions = [
    { username_norm: "23cs200", candidate_id: "23CS200", name: "Devan R K", created_at: "2026-06-10T04:20:00.000Z" }
  ];
  ...
  assert.equal(unmatchedRow.candidate_id, "23CS200"); // typed display id from the submission denorm
  assert.equal(unmatchedRow.name, "Devan R K");     // name typed at login (session doc)
  ...
  assert.equal(unmatchedRow.username_norm, "23cs200"); // forensic key preserved
  ...
  const csv = buildResultsCsv(rows, [{ problem_id: "p1", title: "Sum Two" }, { problem_id: "p2", title: "Reverse" }]);
  assert.match(csv, /^1,23CS200,Devan R K,,150,100,50,0,0,0,none,none,,,,,,,yes$/m);
});
...
test("buildResultsRows: MIXED keying — enrolled row joins by person_id, unmatched by username_norm", () => {
  ...
  const sessions = [{ username_norm: "23cs200", candidate_id: "23CS200", name: "Devan", created_at: "2026-06-10T04:20:00.000Z" }];
  const evaluations = new Map([
    ...
    ["23cs200", scorecard({ talent: { composite: 12 }, integrity: { paste_ratio: 0.91 }, tiers: { talent: "weak", integrity: "flag", one_line: "Full paste." },
