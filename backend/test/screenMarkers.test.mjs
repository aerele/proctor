...
//   - contest snapshot field: person-contest sessions read the CONTEST's
//     screen_markers field (template defaults -> instantiate -> contest doc),
//     default OFF, garbage falls back to disabled.
...
//     below — and /api/exam-config is never a screen-markers carrier.
...
function seedSession(firestore, id, overrides = {}) {
  firestore.collection(process.env.SESSION_COLLECTION).doc(id).set({
    ...
    candidate_id: "alice", username_norm: "alice",
    name: "Alice A", roll_number: "R1", email: "a@x.y", room: "Lab A-1",
    contest_slug: "round-1",
    storage_prefix: `contests/round-1/sessions/alice/${id}/`,
...
function seedPersonContest(firestore, slug, overrides = {}) {
  firestore.collection(process.env.CONTESTS_COLLECTION).doc(slug).set({
    ...
    room_gate_enabled: false, rooms: [], problems: [{ problem_id: "sum-two", points: null, order: 0 }],
...
// Start pinned to an OPEN no-roster person contest (the contest carries the
// screen_markers snapshot the start path serves from).
function startReq(extra = {}) {
  return makeReq({ method: "POST", path: "/api/session/start", body: {
    contest: "round-1", candidate_id: "alice", name: "Alice A", roll_number: "R1", email: "a@x.y",
...
test("session start: flag OFF (default) — response key set is EXACTLY today's, no screen_markers byte anywhere", async () => {
  ...
  seedPersonContest(firestore, "round-1"); // no screen_markers on the contest
...
test("session start: flag explicitly OFF on the contest — still byte-identical (no key)", async () => {
  const firestore = makeFakeFirestore();
  __setClientsForTest({ firestore, storage: makeFakeStorage() });
  seedPersonContest(firestore, "round-1", { screen_markers: { enabled: false } });
  const res = await call(startReq());
...
// ---- 2: contest snapshot drives the flag on resume (both directions) --------

test("person-contest resume: the CONTEST's screen_markers drives the key (both directions)", async () => {
  const firestore = makeFakeFirestore();
  __setClientsForTest({ firestore, storage: makeFakeStorage() });
  // Contest ON -> key rides.
  seedPersonContest(firestore, "round-1", { screen_markers: { enabled: true } });
  ...
  // Contest doc with no field -> default OFF -> key absent.
  ...
  assert.ok(!("screen_markers" in off.body),
    "a person contest whose doc has no/OFF screen_markers must omit the key");
});

// ---- 3: exam-config is NEVER a screen-markers carrier -----------------------

test("exam-config (?contest=): no screen_markers leak on the contest branch", async () => {
  const firestore = makeFakeFirestore();
  __setClientsForTest({ firestore, storage: makeFakeStorage() });
