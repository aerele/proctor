...
test("contest-results: post-clear anonymous submitters ride as unmatched rows with scores + unmatched_count (never silently dropped)", async () => {
  ...
  firestore.collection("cr_sessions").doc("anon1").set({
    session_id: "anon1", contest_slug: contest.slug, username_norm: "23cs200",
    candidate_id: "23CS200", person_id: null, name: "Devan R K",
    status: "ended", created_at: "2026-06-10T04:20:00.000Z"
  });
  firestore.collection("cr_submissions").doc("anon-sub").set({
    session_id: "anon1", contest_slug: contest.slug, username_norm: "23cs200",
    candidate_id: "23CS200", person_id: null, problem_id: "p1", score: 100,
  ...
  seedAlert(firestore, { id: "anon-a", contestSlug: contest.slug, personId: "23cs200", severity: "critical" });
  ...
  assert.equal(row.candidate_id, "23CS200");
  assert.equal(row.name, "Devan R K");         // typed-at-login name from the session
  ...
  assert.match(csvRes.body.csv, /23CS200.*,yes$/m);
});
