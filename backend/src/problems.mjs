...
// sql = 82 ("SQL (SQLite 3.27.2)") — verified against the production RapidAPI
// instance on 2026-06-12.
export const LANGUAGE_IDS = { python: 71, cpp: 54, java: 62, javascript: 63, sql: 82 };

export const SUPPORTED_LANGUAGES = Object.keys(LANGUAGE_IDS);

// ---- SQL (language 82) exec composition --------------------------------------
// Judge0's run command for language 82 redirects the SCRIPT FILE into
// `sqlite3 /box/db.sqlite` — the stdin field is DEAD (verified empirically on
// the production instance). A SQL test therefore ships as ONE script:
//   prelude (output formatting) + the test's seed SQL (CREATE+INSERT, stored as
//   the test's `input`) + the candidate's query — with stdin = "".
// The prelude pins sqlite's output shape: space-separated columns, literal
// "NULL" for nulls (both verified honored by 3.27.2).
//
// AUTHORING RULE: expected outputs for SQL tests must be MACHINE-GENERATED by
// running the reference query through this exact composition (local sqlite3 or
// the engine itself) — never hand-typed. composeSqlExecSource is THE single
// source of truth both the exec handlers and authoring tooling go through
// (night-run/sql-conversion/convert.mjs imports it), so the pipeline that
// builds an expected output is byte-identical to the one that judges it.
export const SQL_FORMAT_PRELUDE = '.separator " "\n.nullvalue NULL\n';

export function composeSqlExecSource(seedSql, candidateSql) {
  return `${SQL_FORMAT_PRELUDE}${seedSql}\n${candidateSql}`;
}
