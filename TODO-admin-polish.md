...
## 2026-06-10 feedback round 5 (Karthi voice multipart, TG ~18:12-18:23) — [F10]
1. **Camera recording: YES, record it** (decision on the camera-not-recorded finding): low fps + low res but MUST capture eye movement (looking down repeatedly at a phone/notes) — Karthi's estimate: ~10 fps needed; resolution just enough to see eye direction. Implement as a SEPARATE MediaRecorder on the raw camera stream (no canvas → no tab-hide freeze; that's what killed the old compositor), own GCS chunks (`camera/chunk-*.webm`), video-only (mic already rides the screen recording). Admin SETTING, **default ENABLED** (camera_recording: enabled + fps + width), revisit after real-world trial. Review UI must surface/play the camera stream (session card + recordings source toggle).
2. **Identity key AMENDMENT (supersedes part of the F9 design)**: the PERSON = (college, unique_id) — stable ACROSS contests because candidates sit multiple rounds. (person × contest) keys scores/attempts/sessions. College becomes first-class. The F9 design's contest-scoped identity is amended accordingly.
3. **Roster**: duplicate unique-ids → REJECT the whole CSV with line numbers + the repeated id (confirms F9 design Q1: strict). **College name = compulsory column** (it forms the person key with unique_id).
4. **Multi-round management**: conduct round N from round N-1's data — select a subset of an already-uploaded roster OR upload a fresh CSV that auto-LINKS to existing persons (same college+unique_id). Keep simple. Cross-contest score view per person (all contests they attempted).
5. **Contest templates → instances (standard model)**: a TEMPLATE = group of questions + settings + rules, named, stored in a list; instantiate a template into an actual CONTEST (a real round) with its own student list, rooms, start/stop, lifecycle, filters. Rooms per contest; **invigilator links unique per contest; student portal URL per contest**. Research how standard platforms (HackerRank etc.) structure this — STANDARD, not patched.
...
## 2026-06-11 friend's live-test feedback (Karthi forwarded 3 voice msgs, TG ~02:50–03:11) — [F12]
Friend tested the deployed dev build (2026-06-10); three items. **EXAM IS TODAY (700 candidates)** — F12.1 is a hard blocker, F12.2 is exam-critical UX, F12.3 is high-value. Do F12.1 → F12.2 → F12.3 BEFORE the remaining wave-6/7 polish; nothing from the existing plan is dropped (Results/People/lifecycle follow). Karthi: "none of the current to-dos or plans disrupted, nothing missed."

1. **F12.1 — Email field bounces candidate OUT of fullscreen (BLOCKER).** On the details page (stage 3 DETAILS, already fullscreen) clicking the Email field exits fullscreen → re-prompts "enter fullscreen". Repeatable; ONLY the email field; all other fields fine. Friend's hypothesis: the browser autofill/address dropdown on email is the trigger. ROOT CAUSE in code: the shared `Field` input (App.tsx:5617) has NO `autocomplete` attribute, so Chrome runs its email/address autofill heuristic + popup. **DECISION (Karthi: "you decide") — kill autofill on all detail fields:** `autoComplete="off"` on `Field`; for the email field render `type="text" inputMode="email"` (Chrome's email-autofill heuristic never fires) + obscure/randomized `name` + `data-1p-ignore` + `data-lpignore="true"` + `data-form-type="other"` (suppress password-manager extensions). Defensive: a transient fullscreen loss during PRE-EXAM stages (PERMISSIONS/FULLSCREEN/DETAILS) must never be punitive and never wipe the typed form (it isn't today — form is React state surviving the gate overlay, and status≠"recording" so the enforcement reducer no-ops; verify both). Reproduce + verify the fix in a real browser before sign-off.

2. **F12.2 — Per-problem, per-language starter-code STUBS (exam-critical UX).** Like HackerRank/CodeChef/Codeforces: each problem ships a prefilled stub per supported language so the candidate fills only the logic inside the function, not imports + `main`. TODAY: generic read-stdin/print-stdout scaffolds exist (`STARTERS` in CodingWorkspace.tsx) but are NOT problem-specific. BUILD: optional `stubs:{python,cpp,java,javascript}` on the problem definition (admin authors in ProblemBank per language); carried in the template snapshot AND the candidate session `problems[]` payload; the editor loads `problem.stubs[lang]` when present, else the generic `STARTERS[lang]`; language switch swaps the stub only if the buffer is still the untouched stub (reuse the existing untouched-rule). Back-compat: problems without stubs behave exactly as today.

3. **F12.3 — Library/function AUTOCOMPLETE (IntelliSense) in the editor.** Candidates shouldn't memorize library function names — autocomplete functions of imported libraries; we test problem-solving, not memory. TODAY: Monaco gives only buffer word-completion. SCOPE CALL (exam today): full language-server IntelliSense per language is too heavy/risky to ship before today; **v1 = curated `monaco.languages.registerCompletionItemProvider` symbol sets** per language covering common stdlib/competitive libraries (Python: math/collections/itertools/heapq/bisect/sys/string; C++ STL via `<bits/stdc++.h>`: vector/map/set/algorithm + common funcs; Java: java.util.* + Scanner/System; JS: Array/Math/string methods) + keep Monaco's built-in suggestions on. Full LSP-backed IntelliSense = post-exam follow-up. Tradeoff flagged to Karthi.
