...
// Sensible defaults: every NOTABLE entry visible (alerts, submissions, errors),
// neutral/info entries HIDDEN (showInfoActivities:false) so trivial proctor
// signals don't clutter the log on load — the "Show info activities" chip reveals
// them. Kind toggles + type/severity narrowing all start wide-open.
...
// LOG-1: "info" (neutral) activity — trivial proctor signals that clutter the log
// on load without telling a reviewer anything actionable: window focus changes,
// fullscreen toggles, clipboard pings, tab visibility, session start, and the
