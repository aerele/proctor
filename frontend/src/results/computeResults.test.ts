...
describe("buildResultsCsv", () => {
  ...
  it("KPR 2026-06-12: unmatched rows export with the yes flag", () => {
    const rows = [row({ person_id: "", username_norm: "23cs200", candidate_id: "23CS200", name: "Devan R K", college: "", total: 100, unmatched: true })];
    const csv = buildResultsCsv(rows, [{ problem_id: "p1", title: "Sum Two" }]);
    expect(csv.split("\n")[1]).toBe("1,23CS200,Devan R K,,100,80,0,0,0,none,none,yes");
  });
...
describe("countUnmatched", () => {
  it("counts only rows flagged unmatched", () => {
    const rows = [...ROWS, row({ person_id: "", username_norm: "23cs200", candidate_id: "23CS200", unmatched: true })];
...
describe("filterResultRows with unmatched rows", () => {
  it("unmatched rows pass through filters like any other row (never silently hidden)", () => {
    const rows = [...ROWS, row({ person_id: "", username_norm: "23cs200", candidate_id: "23CS200", name: "Devan R K", college_norm: "", college: "", total: 100, room: "", unmatched: true })];
    ...
    expect(filterResultRows(rows, { search: "devan" }).map((r) => r.candidate_id)).toEqual(["23CS200"]);
  });
});
